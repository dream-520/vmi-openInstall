 ...
 *
 */
@Service("androidAXiaomiPush")
public class AndroidAXiaoMiPushServiceImpl extends AbstractMiPushService{

	private static final Log logger = LogFactory.getLog(AndroidAXiaoMiPushServiceImpl.class);

	private static final String androidAccountSecret = PushConstant.android_a_xiaomi_accountSecret;
	private static final String androidAccountPackageName = PushConstant.android_a_xiaomi_accountPackageName;
	...
	protected Message buildMessage(String msg, long sec, String msgType, int notifyid) {
		...
		if(pushMessageDto.getMsgType() == PushTypeEnum.type_web_H5.getCode()) {
			message = new Message.Builder()
					...
					.restrictedPackageName(Tools.isNotNull(pushMessageDto.getPackageName())?pushMessageDto.getPackageName():androidAccountPackageName)
