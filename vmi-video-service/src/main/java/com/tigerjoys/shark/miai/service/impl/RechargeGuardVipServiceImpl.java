...
public class RechargeGuardVipServiceImpl implements IRechargeGuardVipService {
	...
	public Map<String, Object> getVipPriceList(String mobileModel) throws Exception {
		...
		if (Tools.isNotNull(categoryEntity)) {
			if (categoryEntity.getId() == 4) {
				outMap = getPriceList(0, 12);
			} else if (categoryEntity.getId() <= 6) {
				outMap = getPriceList(0, 2);
			} else if (categoryEntity.getId() <= 8) {
				outMap = getPriceList(0, 10);
			} else {
				outMap = getPriceList(0, 11);
			}
		} else {
			outMap = getPriceList(0, 2);
		}
	...
	public Map<String, Object> getPriceList(long anchorId,int type) throws Exception {
		...
		if( type == 1){
		...
		}else if( type == 2 || type == 3 || type == 6 || type == 7 || type == 8 || type == 9 || type == 10 || type == 11 || type == 12){
			if (os == 1) {
				map.put("vipInfo1", "开通尊贵服务功能" + "\n更赠送100元话费");
			} else {
				map.put("vipInfo1", "开通尊贵服务功能" + "\n享受更多权益");
			}
			...
			map.put("buyVipNum", "已有"+bSequenceContract.getCurrentValue("vipBuy", 0L)+"人购买");
			map.put("vipInfo3", "尊贵服务特权");
			...
			Collections.shuffle(rechargeInfoList);
		...
		*/
		...
		if (Tools.isNotNull(list)) {
			...
			for (GuardVipCategoryEntity price : list) {
				...
				if( (type==2 || type==10 || type==12  ) && "FBL_20200318_1".equals(channel) && price.getDays() ==30 ){
					price.setMoney(1800); //渠道 FBL_20200318_1 的VIP 其它、<1500 1500~2500这3个档 1月为18元
				}
	...
	public ActionResult recharge(long priceId, int channelCode, double money, double income, Map<String,String> ipaParam,Long anchorId) throws Exception {
		...
		if( (price.getType()==2 || price.getType()==10 || price.getType()==12  ) && "FBL_20200318_1".equals(channel) && price.getDays() ==30 ){
			price.setMoney(1800); //渠道 FBL_20200318_1 的VIP 其它、<1500 1500~2500这3个档 1月为18元
		}
		...
		if(price.getType() == 1){
			...
			buyInfo = "守护充值" ;
				...
				|| price.getType() == 7 || price.getType() == 8 || price.getType() == 9 
				|| price.getType() == 10 || price.getType() == 11 || price.getType() == 12
				){
