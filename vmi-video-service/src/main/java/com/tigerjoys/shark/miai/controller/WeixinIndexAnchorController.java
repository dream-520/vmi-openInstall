	...
	public String index(Model model) throws Exception {
		...
		if(Tools.isNotNull(query)) {
			...
			if(Tools.isNotNull(sets)) {
				...
				for (Tuple tuple : sets) {
					if(Tools.isNotNull(tuple)) {
						...
						if(Tools.isNotNull(data)) {
							...
							if(Tools.isNotNull(anchor) && anchor.getUserid() != 154392008136392960L) {
								//配置数据
				...
				if(Const.IS_TEST) {
					anchor = anchorOnlineContract.findByProperty("userid", 65418693063999232L);
				} else {
					anchor = anchorOnlineContract.findByProperty("userid", 154392008136392960L);
				}
