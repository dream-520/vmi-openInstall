...
//@TestEncrypt("")
...
public class WebActivityAnchorStar {
	...
	public String anchorStarPop(Model model) throws Exception {
		...
		pageModel.clearAll();
		pageModel.addLimitField(0, 1);
		pageModel.addQuery(Restrictions.eq("issue_state", 0));
		pageModel.asc("id");
		activitys = activityStarIssueContract.load(pageModel);
		long surplus = 0;
		if(Tools.isNotNull(activitys)) {
			ActivityStarIssueEntity activity = activitys.get(0);
		...
		int hours = (int) (surplus / 3600);
		int min = (int) ((surplus -(hours * 3600)) / 60);
		StringBuilder sb = new StringBuilder();
		if(hours > 0) {
			sb.append(hours + " 小时 " + min + " 分钟");
		} else {
			if( min == 0)
				min = 1;
			sb.append(min + " 分钟");
		}
		model.addAttribute("surplus", sb.toString());
		...
		long count = activityStarUserVoteContract.count(pageModel);
		//0 未投注 1 投注未中奖 2 投注中奖了
		int vote = 0;
		//中奖钻石值
		int diamod = 0;
		if(count > 0) {
		...
		System.err.println("中奖信息"+vote +"中奖钻石:"+diamod);
		model.addAttribute("vote", vote);
		model.addAttribute("diamod", diamod);
		return "activity/goddess/homeShow";
	}
		...
		if(Tools.isNotNull(json)) {
			...
			if(issue > 0 && anchorid > 0) {
				...
				if(Tools.isNotNull(activity)) {
					...
					long surplus = ( end - current ) / 1000;
					//检测活动结束五分钟之前不能进行投票操作处理
					if(surplus >= 300) {
						//进行尝试扣除小红花数量处理
