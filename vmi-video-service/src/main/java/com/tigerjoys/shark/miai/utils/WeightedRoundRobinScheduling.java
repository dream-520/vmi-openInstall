...
public class WeightedRoundRobinScheduling {
    ...
    private List<WeighttedServer> serverList; //服务器集合
     ...
     * @return
     */
    private static int getGCDForServers(List<WeighttedServer> serverList ) {
        int w = 0;
     ...
     * @return
     */
    public static int getMaxWeightForServers(List<WeighttedServer> serverList) {
        int w = 0;
        for (int i = 0, len = serverList.size(); i < len - 1; i++) {
            if (w == 0) {
     ...
     */
    public WeighttedServer GetServer() {
        while (true) {
    ...
    public WeightedRoundRobinScheduling(List<WeighttedServer> list) {
        serverList =list;
        currentIndex = -1;
        currentWeight = 0;
    ...
    public static void main(String[] args) {
    	WeighttedServer s1 = new WeighttedServer("A", 23);//3
    	WeighttedServer s2 = new WeighttedServer("B", 23);//2
    	WeighttedServer s3 = new WeighttedServer("C", 20);
    	WeighttedServer s4 = new WeighttedServer("D", 34);
//    	WeighttedServer s3 = new WeighttedServer("192.168.0.102", 6);//6
//    	WeighttedServer s4 = new WeighttedServer("192.168.0.103", 4);//4
//    	WeighttedServer s5 = new WeighttedServer("192.168.0.104", 1);//1
        ArrayList<WeighttedServer> list=new ArrayList<WeighttedServer>();
        list.add(s1);
        list.add(s2);
        list.add(s3);
        list.add(s4);
...
//        list.add(s4);
//        list.add(s5);
       WeightedRoundRobinScheduling obj = new WeightedRoundRobinScheduling(list);
        
        Map<String,Integer> countResult = new HashMap<String,Integer>();
        
        for (int i = 0; i < 100; i++) {
        	WeighttedServer s = obj.GetServer();
            String log = "ip:"+s.name+";weight:"+s.weight;
